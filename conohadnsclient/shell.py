...
from conohadnsclient import utils
from conohadnsclient.version import version_info as version
...
class ConoHaDnsShell(App):
    CONSOLE_MESSAGE_FORMAT = '%(levelname)s: %(message)s'
    DEFAULT_VERBOSE_LEVEL = 0

    def __init__(self):
        super(ConoHaDnsShell, self).__init__(
            description='ConoHaDns Client',
            version=version.version_string(),
            command_manager=CommandManager('conohadnsclient.cli'),
        )

        self.log = logging.getLogger(__name__)

    def build_option_parser(self, description, version):
        parser = super(ConoHaDnsShell, self).build_option_parser(
            description, version)
    ...
    def configure_logging(self):
        ...
        super(ConoHaDnsShell, self).configure_logging()
    ...
    def initialize_app(self, argv):
        super(ConoHaDnsShell, self).initialize_app(argv)
    ...
    def run(self, argv):
        try:
            return super(ConoHaDnsShell, self).run(argv)
